...
Matrix createMatrix(int rows, int cols) {
    Matrix M = (Matrix)_mm_malloc(rows * cols * sizeof(float), 16); // Aligned malloc
    if (M == NULL)
...
void matVecMult(Matrix A, Matrix B, Matrix C, int rows, int cols) {
    int i, k;
    __m128 sum, a_val, b_val;
    
    int simd_end = cols - (cols % 4); // This computes how many elements can be processed with SIMD
    
    for (i = 0; i < rows; i++) {
        sum = _mm_setzero_ps();
        
        // SIMD part
        for (k = 0; k < simd_end; k += 4) {
            a_val = _mm_loadu_ps(&A[i * cols + k]);
            b_val = _mm_loadu_ps(&B[k]);
            sum = _mm_add_ps(sum, _mm_mul_ps(a_val, b_val));
        }
        
        // Sum up the elements of sum
        float temp[4];
